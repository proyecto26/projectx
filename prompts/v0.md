...
- **Component Review**: Review components in the `packages/ui/src/lib` directory. Create directories if needed for grouping similar components (e.g., `navigation`, `icons`, `layouts`, etc.).
...
- **Styling**: Use Tailwind CSS and DaisyUI for styling. Utilize utility classes directly in the JSX.
  - For complex styles, consider using the `classnames` function from `packages/ui/src/utils/tailwind.ts` to conditionally apply classes (already uses `clsx` and `tailwind-merge` internally).
